...
use mfrc522::{
    MifareKey,
    comm::{Interface, blocking::spi::SpiInterface},
};

use crate::rfid::{
    BLOCK_USIZE, CARD_USIZE, Error, Reader, SAFE_SECTOR_USIZE, SECTOR_SIZE, SECTOR_USIZE,
    SelectedCard,
};
...
pub fn read_block<'s, 'r, E: Debug, COMM: Interface<Error = E>>(
    selected: &'s mut SelectedCard<'r, E, COMM>,
    block: u8,
    key: &MifareKey,
) -> Result<[u8; BLOCK_USIZE], Error<E>> {
...
pub fn read_sector<'s, 'r, E: Debug, COMM: Interface<Error = E>>(
    selected: &'s mut SelectedCard<'r, E, COMM>,
    sector: u8,
    key: &MifareKey,
) -> Result<[u8; BLOCK_USIZE * SECTOR_USIZE], Error<E>> {
...
pub fn read_card<'s, 'r, E: Debug, COMM: Interface<Error = E>>(
    selected: &'s mut SelectedCard<'r, E, COMM>,
    keys: &[MifareKey; CARD_USIZE],
) -> Result<[u8; BLOCK_USIZE * SECTOR_USIZE * CARD_USIZE], Error<E>> {
...
pub fn write_block<'s, 'r, E: Debug, COMM: Interface<Error = E>>(
    selected: &'s mut SelectedCard<'r, E, COMM>,
    block: u8,
    key: &MifareKey,
    data: [u8; BLOCK_USIZE],
...
pub fn write_sector<'s, 'r, E: Debug, COMM: Interface<Error = E>>(
    selected: &'s mut SelectedCard<'r, E, COMM>,
    block: u8,
    key: &MifareKey,
